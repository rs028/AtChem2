...
LOG_FILE=tests/oldtests.log

echo "Executing tests script." > $LOG_FILE
echo "Tests to run:" $1 >> $LOG_FILE
echo "" >> $LOG_FILE
...
=======
  ./build/build_atchem2.sh --chemfile=$TESTS_DIR/$test/model/configuration/$test.fac --mechanism=$TESTS_DIR/$test/model/configuration/include --configuration=$TESTS_DIR/$test/model/configuration --mcm=v3.3.1 &> /dev/null
>>>>>>> Correct model tests output
  exitcode=$?
...
=======
  ./atchem2 --output=$TESTS_DIR/$test/output --mechanism=$TESTS_DIR/$test/model/configuration/include --configuration=$TESTS_DIR/$test/model/configuration --constraints=$TESTS_DIR/$test/model/constraints > $TESTS_DIR/$test/$test.out 2>&1
>>>>>>> Correct model tests output
...
$this_file_failures"
  ...
  # to logfile (tests.log). Increment the counters as necessary.
  if [ -z "$this_test_failures" ]; then
    ...
    echo "-> test:" $test "FAILED" >> $LOG_FILE
    echo "" >> $LOG_FILE
    echo "*" $test
